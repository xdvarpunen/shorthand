...
class AtticNumeralsPage extends StatefulWidget {
  const AtticNumeralsPage({super.key});

  @override
  State<AtticNumeralsPage> createState() => _AtticNumeralsPageState();
}

class _AtticNumeralsPageState extends State<AtticNumeralsPage> {
...
class EtruscanNumeralsLinesInterpreter extends TextLinesInterpreter {
  ...
  bool isNumberFive(List<Line> lines) {
    ...
    final horizontals = lines
        .where(
          (line) =>
              !verticals.contains(line) && LineInspector.isHorizontalLine(line),
        )
        .toList();
    if (horizontals.length != 1) return false;
    final horizontal = horizontals.first;

    // Horizontal must intersect both verticals
    if (!LineInspector.linesIntersect(horizontal, verticals[0])) return false;
    if (!LineInspector.linesIntersect(horizontal, verticals[1])) return false;

    // Verticals should not intersect each other
    if (LineInspector.linesIntersect(verticals[0], verticals[1])) return false;

    return true;
  }

  bool isNumberThousand(List<Line> lines) {
  ...
  String _detectNumber(List<Line> lines) {
    ...
    if (isNumberThousand(lines)) return '1000';
