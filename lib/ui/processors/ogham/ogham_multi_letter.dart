...
import 'package:shorthand_app/common/core/segment_intersection_grouper.dart';
import 'package:shorthand_app/common/core/stroke_direction.dart';
import 'package:shorthand_app/common/core/stroke_util.dart';
import 'package:shorthand_app/ui/processors/ogham/ogham_script_util.dart';

class OghamMultiLetterProcessor extends TextLinesInterpreter {
  ...
  String processSingleLetter(List<List<Point>> lines) {
    if (lines.length == 1) {
      if (isUi(lines)) {
        return "Ui ᚗ";
      } else if (isOi(lines)) {
  ...
  List<List<Point>> toListOfListOfPoints(List<Line> lines) {
    return lines.map((line) => line.points).toList();
  }

  @override
  String process(List<Line> lines) {
    final allLines = toListOfListOfPoints(lines);
    ...
    for (final letterLines in letters) {
      final result = processSingleLetter(letterLines);
