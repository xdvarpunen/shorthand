...
class TallyMarksTwoProcessorService extends CanvasProcessor {
  ...
  int totalCount(List<List<Point>> lines) {
    ...
    for (var index = 0; index < intersectingLines.length; index++) {
      ...
      if (groupOfIntersectingLines.length == 1) {
        var nonIntersectingHorizontalLines = findNonIntersectingHorizontalLines(
          groupOfIntersectingLines,
        );
        count += nonIntersectingHorizontalLines.length;

        var nonIntersectingVerticalLines = findNonIntersectingVerticalLines(
          groupOfIntersectingLines,
        );
        count += nonIntersectingVerticalLines.length;
      } else if (groupOfIntersectingLines.length == 2) {
        var validPairs2 = findSpecialIntersectingLines(
          groupOfIntersectingLines,
        );
        count += validPairs2.length * 2;
      } else if (groupOfIntersectingLines.length == 3) {
        var validPairs3 =
            findVerticalLinesIntersectingTwoIsolatedHorizontalLines(
              groupOfIntersectingLines,
            );
        count += validPairs3.length * 3;
      } else if (groupOfIntersectingLines.length == 5) {
        var validPair5 = LineProcessor3().checkLineConditions(
          groupOfIntersectingLines,
        );
        if (validPair5) {
          count += 5;
...
class LineProcessor3 {
  ...
  bool checkLineConditions(List<List<Point>> lines) {
    ...
    var verticalLineToHorizontalConnections = verticalLines.map((verticalLine) {
      return horizontalLines
          .where(
            (horizontalLine) => _lineIntersectionUtil.linesIntersect(
              verticalLine,
              horizontalLine,
            ),
          )
    ...
    bool isFirstVerticalLineConnectedToOneHorizontal =
        verticalLineToHorizontalConnections[0] == 1 ||
        verticalLineToHorizontalConnections[0] == 3;
    bool isSecondVerticalLineConnectedToThreeHorizontals =
        verticalLineToHorizontalConnections[1] == 1 ||
        verticalLineToHorizontalConnections[1] == 3;

    return isFirstVerticalLineConnectedToOneHorizontal &&
        isSecondVerticalLineConnectedToThreeHorizontals;
  }
}
